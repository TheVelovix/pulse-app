...
export default function PrivacyPolicy(){
  ...
  return (
    <ScrollView style={{flex:1, paddingHorizontal:10}}>
      <Pressable onPress={() => router.back()} style={({pressed}) => [policyStyles.smallBackButton, pressed && {backgroundColor:"rgb(150, 150, 150)"}]}>
      ...
      <View style={policyStyles.policyWrapper}>
        {
          privacyPolicy.map((section, index) => {
            return (
              <View key={index} style={index < privacyPolicy.length ? policyStyles.sectionBorder : {}}>
                <Text style={policyStyles.sectionTitles}>{index + 1} {section.title}</Text>
